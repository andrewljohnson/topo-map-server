...
"""
import sys,json,math,multiprocessing,argparse
...
def initialize(path):
 ...
 base=Path(path)
def run(key):
 x,y=key;findings=[];counts=Counter();original=__import__("trail_matching").conflate
...
if __name__=='__main__':
 parser=argparse.ArgumentParser(description=__doc__);parser.add_argument('--parents',required=True,type=Path);parser.add_argument('--basemap',required=True,type=Path);parser.add_argument('--output',required=True,type=Path);parser.add_argument('--workers',type=int,default=16);args=parser.parse_args()
 ...
 with ProcessPoolExecutor(max_workers=args.workers,mp_context=multiprocessing.get_context('spawn'),initializer=initialize,initargs=(args.basemap,)) as pool:
