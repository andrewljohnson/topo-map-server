...
export default function TahoeExperiment(){
 ...
 const [status,setStatus]=useState('Loading sample…'),[error,setError]=useState('');
 ...
 (async()=>{try{
  ...
  let first=true;map.on('idle',()=>{if(first){first=false;setStatus(`Ready in ${((performance.now()-start)/1000).toFixed(1)}s · vectors + elevation`);}});
 ...
 <div style={{display:'flex',gap:8,marginTop:10}}>{[['Fallen Leaf',-120.061,38.907],['South shore',-120.005,38.95]].map(([label,lng,lat])=><button key={label} onClick={()=>mapRef.current?.flyTo({center:[lng,lat],zoom:14})} style={{border:'1px solid #a5b9ae',borderRadius:7,padding:'7px 10px',background:'white',fontSize:14}}>{label}</button>)}</div>
 </aside></main>;
}
