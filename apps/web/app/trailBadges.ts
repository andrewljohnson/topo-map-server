...
export function installTrailBadges(map:any){
 ...
 function add(ref:string){const id='trail-'+ref;if(!colors[ref]||map.hasImage(id))return;const canvas=document.createElement('canvas');canvas.width=80;canvas.height=80;const ctx=canvas.getContext('2d')!;
