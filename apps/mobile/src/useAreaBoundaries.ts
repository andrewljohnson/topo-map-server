...
export function useAreaBoundaries(api:string,onData:(data:AreaData)=>void,enabled=true){
 const callback=useRef(onData);callback.current=onData;
 useEffect(()=>{if(!enabled)return;let stopped=false;const controller=new AbortController();const cacheFile=FS.documentDirectory+'area-boundaries.json';
