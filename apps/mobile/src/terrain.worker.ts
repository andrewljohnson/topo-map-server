...
const ctx:any=self,pending=new Map<number,any>(),jobs=new Map<number,AbortController>();let sequence=0;
...
ctx.onmessage=async(event:any)=>{const m=event.data;
 ...
 try{const levels=m.z>=14?[20,100]:m.z>=13?[40,200]:m.z>=12?[100,500]:[200,1000];
  const result=await manager.fetchContourTile(m.z,m.x,m.y,{levels,multiplier:1/.3048,overzoom:1,contourLayer:'contour',elevationKey:'ele_ft',levelKey:'level',buffer:2,subsampleBelow:128},controller);
