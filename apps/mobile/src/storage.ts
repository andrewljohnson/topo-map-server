...
export class TileStore{
 ...
 constructor(public api:string,public changed:()=>void,private options:{retryDelayMs?:number;progressMs?:number;requestTimeoutMs?:number}={}){}
 ...
 async init(){await FS.makeDirectoryAsync(root,{intermediates:true});try{const data=JSON.parse(await FS.readAsStringAsync(statePath));if(data.api===this.api){this.meta=data.meta;this.regions=data.regions||{};this.nativePending=data.nativePending||{};for(const r of Object.values(this.regions))if(r.status==='downloading')r.status='queued';}}catch{};try{const response=await fetch(this.api+'/metadata',{signal:AbortSignal.timeout(8000)});if(!response.ok)throw Error('Metadata '+response.status);const latest:Metadata=await response.json();if(!latest.datasetId)throw Error('Vector metadata needs datasetId');if(composition(latest)!==composition(this.meta)){
 ...
 private async nativeBatch(url:string,jobs:Job[],signal:AbortSignal){
  ...
  await this.save();
  const task=FS.createDownloadResumable(url,path,{sessionType:FS.FileSystemSessionType.BACKGROUND});
 ...
 }catch(e){if(flight.yielded)break;if(attempt===2||controller.signal.aborted){for(const job of remaining)this.finish(job,e instanceof Error?e:Error(String(e)));remaining=[]}}remaining=remaining.filter(j=>this.requests.has(j.path));if(remaining.length)await new Promise(r=>setTimeout(r,(this.options.retryDelayMs??500)*2**attempt))}
