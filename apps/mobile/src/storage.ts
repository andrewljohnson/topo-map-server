...
export class TileStore{
 ...
 tileUrl(key:string){const parsed=this.parse(key);const[z,x,y]=parsed.key.split('/');const path=(this.set(parsed.tileset)?.tileUrl||'/tiles/{z}/{x}/{y}.pbf').replace('{z}',z).replace('{x}',x).replace('{y}',y);return new URL(path,this.api+'/').href;}
