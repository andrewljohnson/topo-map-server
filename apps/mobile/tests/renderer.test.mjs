...
const extraModules=Object.fromEntries(['trailBadges','featureInfo','poiMatching','mapNotes','terrainRuntime','terrainWorkerSource'].map(name=>[name,'data:text/javascript;base64,'+Buffer.from(transpile(fs.readFileSync(new URL('../src/'+name+'.ts',import.meta.url),'utf8'))).toString('base64')]));
const mapSource=transpile(fs.readFileSync(new URL('../src/map.ts',import.meta.url),'utf8')).replace(/import .*? from ['"]\.\/mapAssets['"];?/,"const maplibreJs='',maplibreCss='',maplibreWorker='';").replace("'./shieldImages'",JSON.stringify(shieldModule)).replace("'./poiImages'",JSON.stringify(poiModule)).replace("'./amenityImages'",JSON.stringify(amenityModule)).replace("'./mapInfo'",JSON.stringify(mapInfoModule)).replace("'./trailBadges'",JSON.stringify(extraModules.trailBadges)).replace("'./featureInfo'",JSON.stringify(extraModules.featureInfo)).replace("'./poiMatching'",JSON.stringify(extraModules.poiMatching)).replace("'./mapNotes'",JSON.stringify(extraModules.mapNotes)).replace("'./terrainRuntime'",JSON.stringify(extraModules.terrainRuntime)).replace("'./terrainWorkerSource'",JSON.stringify(extraModules.terrainWorkerSource));
