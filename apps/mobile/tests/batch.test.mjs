...
test('cancelling stops scheduled chunks and aborts requests; saved queue resumes',async()=>{memory.clear();const net=network();const store=new TileStore('http://batch3',()=>{},{retryDelayMs:1});await store.init();await store.toggle(cell);await delay(15);await store.toggle(cell);await idle(store);assert.equal(store.regions[cell],undefined);assert.ok(net.calls.length<=2);assert.ok(net.aborted>0);memory.set('memory://topo-vectors-v2/state.json',JSON.stringify({api:'http://batch3',meta,regions:{[cell]:{status:'downloading',done:5,total:27}}}));const resumed=new TileStore('http://batch3',()=>{},{retryDelayMs:1});await resumed.init();await idle(resumed);assert.equal(resumed.regions[cell].status,'complete');assert.equal(resumed.regions[cell].done,27);});
