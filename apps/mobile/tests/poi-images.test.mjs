...
const scope=vm.createContext({Path2D:class{constructor(d){this.d=d}},document:{createElement:()=>({getContext:()=>ctx})}});vm.runInContext(code,scope);
...
assert.equal(images.size,0);events.styleimagemissing({id:'poi-square-cafe'});assert.equal(images.size,1);events['style.load']();assert.equal(images.size,56);assert.ok(pathDraws>=56);
...
const draws=pathDraws;events['style.load']();events.styleimagemissing({id:'poi-square-unknown'});events.styleimagemissing({id:'shield-us'});assert.equal(pathDraws,draws);images.clear();events['style.load']();assert.equal(images.size,56);
