...
Source-connected agency junctions are preserved during conflation. When a branch originally joins an agency segment that has an established OSM replacement, a short connector may join that unchanged branch to the matched reference within the existing match tolerance. Mere proximity, grade-separated crossings and original OSM gaps do not qualify. Tiny survey excursions whose two newly cut ends collapse onto one reference point are discarded instead of becoming false spurs. `experiments/tahoe/audit_connectivity.py` reports original dead ends, disconnected new cuts and lost source-junction candidates separately for bounded review.

The junction rule requires a real branch: within20m of the shared source endpoint, at least5m of the partner must leave the first line’s1m survey corridor. Duplicate survey termini cannot authorize a connector. Short clipped terminal stubs are removed only when the remaining endpoint already lies on the replacement network; original short branches remain. The strict640-tile Tahoe audit distinguishes these duplicate termini from genuine branch junctions.

Major-water text uses source importance and horizontal-fit capacity; larger text does not bypass polygon fitting. Lake point labels flip upright after completed rotation gestures without changing coordinates or relayout during dragging. Bridge decks are ordered after road geometry despite early overview/regional-contour labels. The viewport legend evaluates feature-dependent paint; its supported expression subset is checked against MapLibre.

Wider Sierra checks also cover a source junction immediately outside a trimming mask. The original shared endpoint and distinct branch remain required; the mask must be within its established matching tolerance. The closest retained part of that exact source partner is preferred over a farther replacement centerline. Original source gaps and grade-separated crossings remain unchanged. An exactly retraced three-vertex remainder shorter than50m is discarded only when both endpoints were newly cut and snapped to the same anchor; original dead ends and actual noncollapsed loops remain. Derived official-trail revision12 includes these cases.
